...
$$
\text{设 } y_{1}(x), y_{2}(x) \text{ 是对应的二阶齐次线性方程}
$$
...
$$
\text{二维空间中的波动其实还是在三维空间中传播的波动，只是该波动跟坐标 } z \text{ 无关而已。} \\
\text{因此三维波动的泊松公式，消除了坐标 } z \text{，就成为二维波动的公式，这称为降维法。}
$$

$$
\text{对于二维问题，球面 } S_{a t}^{r} \text{ 上的积分应代之以 } xy \text{ 平面的圆 } \Sigma_{a t}^{x, y} \text{ 上的积分。}
$$
